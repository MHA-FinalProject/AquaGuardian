...
public static class TrialDataService
{
    ...
    public static TrialDataModels.TrialData LoadTrialParameters(int trialId, bool useRandomParameters = false, string customPath = null, TextAsset fallbackTextAsset = null)
    {
        try
        {
            ...
            int dataIndex = FindTrialRowIndex(lines, trialId);
    ...
    /**
     * Find trial row index by trial ID
     */
    private static int FindTrialRowIndex(string[] lines, int trialId)
    {
        for (int i = 1; i < lines.Length; i++)
        {
            ...
            {
                if (int.TryParse(searchFields[0], out int parsedTrialId) && parsedTrialId == trialId)
                {
                    return i;
                }
            }
