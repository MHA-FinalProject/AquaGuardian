...
public static class DifficultyParameterSolver
{
        ...
        List<TrialDataModels.TrialData> trials = null)
    {
        ...
        for (int iter = 0; iter < maxIterations; iter++)
        {
            ...
            foreach (var featureName in topFeatures)
            {
                ...
                float oldValueNorm = ApplyNormalization(oldValueRaw, modelIdx, mu, sigma);

                float gradient = coef;
                float step = -adaptiveLR * error * gradient / sumBetaSq;
                float newValueNorm = oldValueNorm + step;

                float newValueRaw = ReverseNormalization(newValueNorm, modelIdx, mu, sigma);
        ...
        Debug.Log($"[DifficultyParameterSolver] Multi fallback final error: {finalError:F2}%");
    ...
    // Helper: Apply normalization if normalizer exists, otherwise return raw value
    ...
    // Helper: Reverse normalization if normalizer exists, otherwise return normalized value
