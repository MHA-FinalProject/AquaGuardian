 ...
 * 
 * FIX: Prevents fish from being replaced by chest between trials
 */
public class TrialFishSpawner : MonoBehaviour
{
    ...
    public GameObject CreateTrialFish(Vector3 position, int trialNumber)
    {
        ...
        {
            Debug.Log($"Cleaning up previous trial fish: {currentTrialFish.name}");
        ...
        {
            Debug.LogWarning($"⚠️ NO FISH PREFAB! Using chestPrefab as last resort: {chestPrefab.name}");
        ...
        if (selectedPrefab != null)
        {
            ...
            Debug.Log($"✓ Created trial target from prefab: {selectedPrefab.name}");
        }
        else
        {
            // Emergency fallback: Create procedural fish (Sphere)
            Debug.LogError("❌ NO PREFAB ASSIGNED! Creating emergency Sphere");
            ...
            Debug.Log("Created procedural sphere fish as emergency fallback");
        }
        ...
        // ✓ CRITICAL: Ensure fish is active!
        fish.SetActive(true);
        ...
        Debug.Log($"✓ Trial fish '{fish.name}' created successfully at {position}, active: {fish.activeSelf}");
        return fish;
    }
    ...
    {
        Debug.Log($"=== SETTING UP FISH TRIGGER FOR {fish.name} ===");
        ...
        if (existingCollider != null)
        {
            ...
            Debug.Log($"Using existing {existingCollider.GetType().Name} as trigger");
        }
        else
        {
        ...
        if (rb == null)
        {
            ...
            Debug.Log("Added Rigidbody to fish");
        }
        rb.isKinematic = true;
        rb.useGravity = false;
        ...
        {
            Debug.Log("BACKUP: Adding GoToEndGame component");
            ...
            if (sceneNameField != null)
            {
                ...
                Debug.Log("Set GoToEndGame sceneName to Win_Scene");
            }
        }
    ...
    private void VerifyTriggerSetup(GameObject fish)
    {
        ...
        Debug.Log($"=== TRIGGER VERIFICATION ===");
        Debug.Log($"Fish tag: '{fish.tag}'");
        Debug.Log($"Fish layer: {fish.layer} ({LayerMask.LayerToName(fish.layer)})");
        Debug.Log($"Fish collider: {fishCollider?.GetType().Name}, isTrigger: {fishCollider?.isTrigger}");
        Debug.Log($"Fish rigidbody: isKinematic: {fishRb?.isKinematic}, useGravity: {fishRb?.useGravity}");
        ...
        if (playerMovement != null)
        {
            ...
            Debug.Log($"Distance to player: {distance:F1} units");
        }
    ...
    /// <summary>
    /// Ensure player has proper trigger detection setup
    /// </summary>
    private void EnsurePlayerTriggerSetup()
    {
        ...
        Debug.Log("Player trigger setup verified");
    }
    ...
    /// <summary>
    /// Cleanup current trial fish
    /// </summary>
    public void CleanupCurrentFish()
    {
        if (currentTrialFish != null)
        {
            Debug.Log($"Cleaning up current trial fish: {currentTrialFish.name}");
            Destroy(currentTrialFish);
            currentTrialFish = null;
        }
    }
    ...
    /// <summary>
    /// Verify fish position (debug tool)
    /// </summary>
    ...
    /// <summary>
    /// Check if fish prefab is assigned (debug)
    /// </summary>
    ...
    public void CheckPrefabStatus()
    {
        ...
        if (fishPrefab == null)
        {
            Debug.LogError("⚠️ WARNING: No global fish prefab assigned! Fish will be replaced by chest!");
        }
    }
